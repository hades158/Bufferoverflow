...
class des():
    ...
    def compute_s_box(self,block,r):
        ###################################your code goes here###################################
        ...
        row = int((block[0]+block[5]),2)#Get the row with the first and last bit
        column = int((block[1]+block[2]+block[3]+block[4]),2)#Get the row with the first and last bit
        val = S_BOX[r][row][column] #Take the value in the SBOX appropriated for the round (i)
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        ###################################your code goes here###################################
        #for each 6 bit subblock you need to apply the corresponding s box 
        # (using the compute_s_box function) and save the result in result value
        ...
        for i in range(len(subblocks)): #For all the sublists
            r = i
            block = subblocks[i]
            block = ''.join(str(ch) for ch in block)
            binary = self.compute_s_box(block,r)
            for x in binary:
                result.append(int(x))
        return result